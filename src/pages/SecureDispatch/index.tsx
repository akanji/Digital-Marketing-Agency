          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center px-1">
              ...
              <button 
                ...
                className="text-[10px] font-black uppercase tracking-widest text-blue-600 hover:text-blue-700 disabled:opacity-50 flex items-center gap-1.5 transition-all"
              >
                {isSanitizing ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Shield className="w-3 h-3" />}
                Sanitize with AI
              </button>
            </div>
            <textarea 
...
const SecureEmailPreview = ({ isOpen, onClose, htmlContent }: { isOpen: boolean, onClose: () => void, htmlContent: string | null }) => {
  return (
    <AnimatePresence>
      {isOpen && (
          ...
          >
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                   <h3 className="text-xs font-black uppercase tracking-widest text-slate-900">Email Manifest Preview</h3>
                   <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Rendered via AOS Preview Engine</p>
                </div>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-slate-100/50">
              {htmlContent ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-400">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-widest">Generating Render...</span>
                </div>
              )}
            </div>
...
export default function SecureDispatch({ onAction }: { onAction: (msg: string, type?: string) => void }) {
  ...
  useEffect(() => {
    const runDiagnostic = async () => {
      try {
        const [healthRes, metricsRes, dispatchesRes] = await Promise.all([
          fetch('/api/health'),
